...
```{r, create-helper-fxns}
## Helper functions

# #' @param plural If `TRUE`, will change “Source:” to “Sources:”
tab_source_urbn <- function(data, source_note, plural = FALSE){
  if (plural) {
    
    data %>% 
      gt::tab_source_note(source_note = gt::md(paste("**Sources:**", source_note)))
    
  } else {
    
    data %>% 
      gt::tab_source_note(source_note = gt::md(paste("**Source:**", source_note)))
    
  }
}

# #` @param plural If `TRUE`, will change “Note:” to “Notes:”
tab_note_urbn <- function(data, source_note, plural = FALSE) {
  if (plural) {
    
    data %>% 
      gt::tab_source_note(source_note = gt::md(paste("**Notes:**", source_note)))
    
  } else {
    
    data %>% 
      gt::tab_source_note(source_note = gt::md(paste("**Note:**", source_note)))
...
basic_table %>% 
  add_housing_table_specific_customizations() %>% 
  tab_source_urbn(source_note = "Made up") %>% 
  tab_note_urbn(source_note = "Narayanan, and Williams, 2019") 
```

<br>
<br>

## Footnotes (Flag for Ben and Christina, do we want to allow this?)
```{r}
basic_table %>% 
  add_housing_table_specific_customizations() %>% 
  tab_footnote(
...
## Source, Notes, and FootNotes together
```{r}
basic_table %>% 
  add_housing_table_specific_customizations() %>% 
  tab_source_urbn(source_note = "Made up") %>% 
